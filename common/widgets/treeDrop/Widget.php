...
namespace common\widgets\treeDrop;


use delikatesnsk\treedropdown\DropdownTreeWidget;
        ...
        if ( is_array( $items ) && count( $items ) > 0 ) {
            foreach( $items as $index => $item ) {
                if (is_object( $item ) && isset( $item->id ) && isset( $item->label ) ) {
                    ...
                    $this->html .= "        ".( isset( $item->items ) && is_array( $item->items ) && ( count( $item->items ) > 0 || $this->ajax !== null ) ? "<i class=\"fa fa-minus-square\"></i>\n" : "" );
                    $this->html .= "        ".( $this->multiSelect ? "<i class=\"fa fa-square\"></i>" : "" );
