                    ...
                    if ( isset( $widget->items ) && is_array( $widget->items ) ) {
                        ...
                        ?>
                            <ul>
                                <li class="parent">
                                    <div class="root node">
                                        <i class="fa fa-<?= ( count( $widget->items ) > 0 ? 'minus' : 'plus' ); ?>-square"></i>
...
$(document).ready(function() {
    $('".$id." .tree ul li .node i.fa-plus-square').each(function(){
    ...
    $('" . $id . " .tree-input .icon .input-clear').on('click', function(e){
        ...
        if ( $('" . $id . " .tree-input > ul').length != 0 ) {
            ...
            $('" . $id . " .tree .node .fa-check-square').removeClass('fa-check-square').addClass('fa-square');
        } else {
...
$js .= "else if ( $(this).hasClass('fa-plus-square') ) {";
...
if ( $widget->ajax !== null && is_array( $widget->ajax ) && isset( $widget->ajax['onNodeExpand'] ) && is_array( $widget->ajax['onNodeExpand'] ) && isset( $widget->ajax['onNodeExpand']['url'] ) && trim( $widget->ajax['onNodeExpand']['url'] ) != '' ) {
    ...
    $js .= "
        ...
        if ( \$node.parent().parent().children('ul').length == 0 ) {
            \$node.removeClass('fa-plus-square').addClass('fa-spinner');
            $.ajax({
                type: '".$method."',
                url: '".$url."',
                data: '".( $ajaxData != "" ? $ajaxData."&" : "" ).$nodeIdParamName."='+( typeof( $(this).parent().children('span').attr('data-id') ) != 'undefined' ? $(this).parent().children('span').attr('data-id') : '' ),
                ...
                success: function (data) {
                    ...
                    if ( $.isArray( data ) && data.length > 0 ) {
                        ...
                        for (var i = 0; i < data.length; i++ ) {
                           ...
                           \$str += '    <li>'+
                                       '       <div class=\"node\">'+
                                       '           <i class=\"fa fa-'+( typeof( data[i].items ) != 'undefined' ? 'plus' : 'minus' )+'-square\"></i>'+
                                       ". ( $widget->multiSelect ? "'           <i class=\"fa fa-square\"></i>'+" : "" )."
                        ...
                        \$node.removeClass('fa-spinner').addClass('fa-minus-square');
                    } else { \$node.remove();  }                
                },
                datatype: 'json',
                async: true
...
if ( $widget->multiSelect ) {
    $js .= "else if ( $(this).hasClass('fa-check-square') ) {
...
if ( $widget->form !== null && ( $widget->form instanceof \yii\widgets\ActiveForm ) &&
    ...
    if ( is_array( $attributes ) && count( $attributes ) > 0 ) {
        foreach ( $attributes as $attribute ) {
            if ( $attribute !== null ) {
                if ( $widget->multiSelect ) {
                    $js .= "$('".$id." .tree ul li .node span[data-id=\"".$attribute."\"]').prev('.fa-square').click();\n";
                } else {
