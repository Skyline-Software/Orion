                ...
                ->widget(\kartik\depdrop\DepDrop::class,[
                    ...
                    'data' => array_map(function ($user){
                             return [$user->id => $user->name];
                                ...
                                ->where(['assn.agency_id'=>$model->agency_id,'assn.role'=>User::ROLE_AGENT])
                                ->all()
                            ),
                    'pluginOptions'=>[
                        'depends'=>['agency-id'],
            ...
            <?= $form->field($model,'user_id')
                ->widget(\kartik\depdrop\DepDrop::class,[
                    ...
                    'data' => array_map(function ($user){
                        ...
                        ->joinWith('agencyAssn assn')
                        ->where(['assn.agency_id'=>$model->agency_id,'assn.role'=>User::ROLE_CUSTOMER])
                        ->all()
                    ),
                    'pluginOptions'=>[
                        'depends'=>['agency-id'],
...
<?php $this->registerJs("
function initMap() {
        var myLatlng = {lat: $model->lat1, lng: $model->lon1};
        var markers = [];
        ...
        markers.push({lat: $model->lat1, lng: $model->lon1});
        markers.push({lat: $model->lat2, lng: $model->lon2});

        var preSetupedFrom = new google.maps.Marker({
                map: map,
                title: 'From',
                position: {lat: $model->lat1, lng: $model->lon1},
                draggable:true,
        });
         var preSetapedTo = new google.maps.Marker({
                map: map,
                title: 'To',
                position: {lat: $model->lat2, lng: $model->lon2},
                draggable:true,
        });

         google.maps.event.addListener(preSetupedFrom, 'dragend', function(location){
            document.getElementById('orderform-lat1').value = location.latLng.lat();
            document.getElementById('orderform-lon1').value = location.latLng.lng();
         });

         google.maps.event.addListener(preSetapedTo, 'dragend', function(location){
            document.getElementById('orderform-lat2').value = location.latLng.lat();
            document.getElementById('orderform-lon2').value = location.latLng.lng();
         });
