...
class UploadAction extends BaseAction
{
            ...
            if ($uploadedFile->error === UPLOAD_ERR_OK) {
                ...
                if (!$validationModel->hasErrors()) {
                    ...
                    if ($path) {
                        ...
                        $this->afterSave($path);
                        if($param = ArrayHelper::getValue($_GET,'type')){
                            $config = ImageHelper::getConfig($param);
                            $w = ArrayHelper::getValue($config,'w');
                            $h = ArrayHelper::getValue($config,'h');
                            $side = ArrayHelper::getValue($config,'bigSide',false);
                            switch ($uploadedFile->type){
                                case 'image/png':
                                    $this->resize_image_png('../../storage/web/source/'.$path, $w, $h,$side);
                                    break;
                                case 'image/svg+xml':
                                    break;
                                default:
                                    $this->resize_image_jpeg('../../storage/web/source/'.$path, $w, $h,$side);
    ...
    public static function findConfig():array
    {
